INSERT INTO Selling_No_Outbound (Bill_Type, Bill_Detail_Id, Bill_Id, Sn, Tenant_Id, Bill_Code, Manual_Code, Bill_Date, Department_Id, Owner_Id, Customer_Id,
	...
	SELECT 202, D.Bill_Detail_Id, B.Bill_Id, D.Sn, B.Tenant_Id, B.Bill_Code, B.Manual_Code, B.Bill_Date, B.Department_Id, B.Owner_Id, B.Customer_Id,
		D.Material_Id, - D.Selling_Quantity, D.Selling_Unit, D.Selling_Packing_Size, - D.Selling_Piece, CAST(0 AS MONEY) - D.Selling_Amount, CAST(0 AS MONEY) - D.Real_Selling_Amount, D.Output_Tax_Rate, CAST(0 AS MONEY) - D.Retail_Amount,
		B.Creator_User_Id, COALESCE(D.COMMENT, B.COMMENT)
	FROM Selling_Return_Bill B
	    INNER JOIN Selling_Return_Bill_Detail D ON B.Bill_Id = D.Bill_Id
    WHERE B.Bill_Id = CURRVAL(CAST('selling_return_bill_bill_id_seq' AS regclass));
...
INSERT INTO Material_Inventory (Material_Id, Tenant_Id, Inventory_Quantity, Inventory_Piece, Inventory_Cost, Available_Inventory_Quantity, Available_Inventory_Piece)
    SELECT D.Material_Id, B.Tenant_Id, D.Selling_Quantity * D.Selling_Packing_Size, D.Selling_Piece, D.Cost_Amount, D.Selling_Quantity * D.Selling_Packing_Size, D.Selling_Piece
    FROM Selling_Return_Bill B
        INNER JOIN Selling_Return_Bill_Detail D ON B.Bill_Id = D.Bill_Id
    WHERE B.Bill_Id = CURRVAL(CAST('selling_return_bill_bill_id_seq' AS regclass))
...
INSERT INTO Business_Account (Bill_Type, Bill_Detail_Id, Bill_Id, Sn, Tenant_Id, Bill_Date, Bill_Code, Manual_Code, Supplier_Customer_Id, Department_Id, Owner_Id, Material_Id,
		...
		Creator_User_Id, Comment)
	SELECT 202, D.Bill_Detail_Id, D.Bill_Id, D.Sn, B.Tenant_Id, B.Bill_Date, B.Bill_Code, B.Manual_Code, B.Customer_Id, B.Department_Id, B.Owner_Id, D.Material_Id,
		- D.Selling_Quantity, D.Selling_Unit, D.Selling_Packing_Size, - D.Selling_Piece, CAST(0 AS MONEY) - D.Cost_Amount, CAST(0 AS MONEY) - D.Selling_Amount, CAST(0 AS MONEY) - D.Real_Selling_Amount, D.Output_Tax_Rate, CAST(0 AS MONEY) - D.Retail_Amount,
...
INSERT INTO AR_Non_receipt (Bill_Type, Bill_Detail_Id, Bill_Id, Sn, Tenant_Id, Bill_Code, Manual_Code, Bill_Date, Department_Id, Owner_Id, Customer_Id, Material_Id,
		...
		Creator_User_Id, Comment)
	SELECT 202, D.Bill_Detail_Id, D.Bill_Id, D.Sn, B.Tenant_Id, B.Bill_Code, B.Manual_Code, B.Bill_Date, B.Department_Id, B.Owner_Id, B.Customer_Id, D.Material_Id,
		- D.Selling_Quantity, D.Selling_Unit, D.Selling_Packing_Size, - D.Selling_Piece, CAST(0 AS MONEY) - D.Selling_Amount, CAST(0 AS MONEY) - D.Real_Selling_Amount, D.Output_Tax_Rate, CAST(0 AS MONEY) - D.Retail_Amount,
		B.Creator_User_Id, COALESCE(D.COMMENT, B.COMMENT)
	FROM Selling_Return_Bill B
	    INNER JOIN Selling_Return_Bill_Detail D ON B.Bill_Id = D.Bill_Id
    WHERE B.Bill_Id = CURRVAL(CAST('selling_return_bill_bill_id_seq' AS regclass));
...
INSERT INTO AR (Customer_Id, Tenant_Id, AR_Balance)
	SELECT B.Customer_Id, B.Tenant_Id, CAST(0 AS MONEY) - SUM(D.Real_Selling_Amount)
	FROM Selling_Return_Bill B
	    INNER JOIN Selling_Return_Bill_Detail D ON B.Bill_Id = D.Bill_Id
	WHERE B.Bill_Id = CURRVAL(CAST('selling_return_bill_bill_id_seq' AS regclass))
...
INSERT INTO AR_Account (Bill_Type, Bill_Id, Tenant_Id, Bill_Code, Manual_Code, Bill_Date, Customer_Id, Receivable_Amount, Creator_User_Id, Comment)
	SELECT 202, B.Bill_Id, B.Tenant_Id, B.Bill_Code, B.Manual_Code, B.Bill_Date, B.Customer_Id, CAST(0 AS MONEY) - SUM(D.Real_Selling_Amount), B.Creator_User_Id, B.COMMENT
	FROM Selling_Return_Bill B
	    INNER JOIN Selling_Return_Bill_Detail D ON B.Bill_Id = D.Bill_Id
    WHERE B.Bill_Id = CURRVAL(CAST('selling_return_bill_bill_id_seq' AS regclass))
