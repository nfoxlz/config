...
SELECT DATE_TRUNC('MONTH', CAST(:Carry_Over_Year_Month AS DATE)) + INTERVAL '1 MONTH', :tenant, Material_Id, SUM(Quantity) - SUM(Out_Quantity), SUM(Piece) - SUM(Out_Piece), :user
...
FROM (SELECT Supplier_Id, Payable_Amount, CAST(0 AS MONEY) Payment_Amount
		FROM AP_Carry_Over
		WHERE Tenant_Id = :tenant
			AND TO_CHAR(Bill_Date, 'YYYYMM') = TO_CHAR(CAST(:Carry_Over_Year_Month AS DATE), 'YYYYMM')
		UNION ALL
...
FROM (SELECT Customer_Id, Receivable_Amount, CAST(0 AS MONEY) Receipt_Amount
		FROM AR_Carry_Over
		WHERE Tenant_Id = :tenant
			AND TO_CHAR(Bill_Date, 'YYYYMM') = TO_CHAR(CAST(:Carry_Over_Year_Month AS DATE), 'YYYYMM')
		UNION ALL
